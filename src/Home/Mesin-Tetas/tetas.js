  ...
  async afterRender() {
    const presenter = new TetasPresenter({
      ...
      onUpdateUI: (data) => {
        ...
        // Helper buat benerin format tanggal PostgreSQL ke ISO agar bisa dibaca JS
        const fixDate = (dateStr) => {
            if (!dateStr || dateStr === 'BATAL' || dateStr === 'null') return null;
            return new Date(dateStr.replace(' ', 'T')); // Force ISO Format
        };
        ...
        data.forEach(item => {
            if (item.status.startsWith('MESIN_')) {
                ...
                valEl.innerText = item.jumlah;

                // LOGIKA TIMER 0/21 HARI
    ...
    document.querySelectorAll('.btn-start-process').forEach(btn => {
      btn.onclick = async (e) => {
        ...
        if (confirm(`Mulai inkubasi 21 hari untuk ${status}?`)) {
