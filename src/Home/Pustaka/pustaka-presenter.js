...
class PustakaPresenter {
  ...
  // Fungsi khusus untuk memberikan efek stabilo (Highlight)
  _highlightText(text, query) {
    if (!query || !text) return text;
    
    // Escaping karakter spesial agar regex tidak error
    const escapedQuery = query.replace(/[.*+?^${}()|[\]\\]/g, '\\$&');
    // 'gi' artinya global dan case-insensitive
    const regex = new RegExp(`(${escapedQuery})`, 'gi');
    
    // Membungkus kata yang cocok dengan span bergaya stabilo kuning
    return text.replace(regex, `<mark style="background-color: #ffeb3b; color: #000; padding: 0 2px; border-radius: 4px; font-weight: 900;">$1</mark>`);
  }
  ...
  _renderTable(data) {
    const query = this.searchInput.value; // Ambil teks pencarian asli
    ...
    this.tableBody.innerHTML = data.map(item => {
      // Terapkan highlight pada Nama dan Deskripsi
      const highlightedNama = this._highlightText(item.nama, query);
      const highlightedDeskripsi = this._highlightText(item.deskripsi || '-', query);
      ...
      return `
        <tr style="border-bottom: 2px solid #eef2ed; background:#ffffff; transition: 0.3s;">
          ...
          <td style="padding: 15px; border-right: 1px solid #f0f0f0; font-weight: 900; text-transform: uppercase; text-align: center; color: #1f3326;">
            ${highlightedNama}
          </td>
