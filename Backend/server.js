...
const init = async () => {
    ...
    handler: async (request, h) => {
        ...
        {
            // 15. GET Semua Data Pembibitan (Lifecycle)
            method: 'GET',
            path: '/api/pembibitan',
            handler: async (request, h) => {
                try {
                    const result = await pool.query('SELECT * FROM pembibitan ORDER BY tanggal_mulai DESC');
                    return { status: 'success', data: result.rows };
                } catch (err) {
                    return h.response({ status: 'error', message: 'Gagal tarik data pembibitan' }).code(500);
                }
            }
        },
        {
            // 16. POST Tambah Batch Baru (Dari Panen ke Fertil)
            method: 'POST',
            path: '/api/pembibitan/add',
            handler: async (request, h) => {
                const { nama, hewan, jumlah, tahapan, tanggal_mulai, pekerjaan } = request.payload;
                try {
                    const query = `
                        INSERT INTO pembibitan (nama, hewan, jumlah, tahapan, tanggal_mulai, pekerjaan) 
                        VALUES ($1, $2, $3, $4, $5, $6) RETURNING *`;
                    const values = [nama, hewan, parseInt(jumlah), tahapan, tanggal_mulai, JSON.stringify(pekerjaan)];
                    const result = await pool.query(query, values);
                    return { status: 'success', data: result.rows[0] };
                } catch (err) {
                    return h.response({ status: 'error', message: 'Gagal buat batch fertil' }).code(500);
                }
            }
        },
        {
            // 17. POST Kualifikasi Lifecycle (Proses Naik Tahap atau Mati)
            method: 'POST',
            path: '/api/pembibitan/qualify',
            handler: async (request, h) => {
                const { id, good, bad } = request.payload;
                try {
                    // 1. Ambil data batch lama
                    const batchRes = await pool.query('SELECT * FROM pembibitan WHERE id = $1', [id]);
                    if (batchRes.rows.length === 0) return h.response({ status: 'fail' }).code(404);
                    
                    const batch = batchRes.rows[0];
                    let nextTahapan = '';
                    
                    // Logic Alur: Fertil -> DOC -> Pullet -> Petelur
                    if (batch.tahapan === 'TELUR FERTIL') nextTahapan = 'DOC';
                    else if (batch.tahapan === 'DOC') nextTahapan = 'PULLET';
                    else if (batch.tahapan === 'PULLET') nextTahapan = 'AYAM PETELUR';

                    // 2. Update Batch Utama (Jumlah yang hidup lanjut ke tahap berikutnya)
                    await pool.query(
                        'UPDATE pembibitan SET jumlah = $1, tahapan = $2 WHERE id = $3',
                        [parseInt(good), nextTahapan, id]
                    );

                    // 3. LOGIC STOK MATI/AFKIR: Masukan ke komoditas lain (Telur Konsumsi / Pedaging)
                    // Disini lu bisa tambahin query INSERT ke tabel komoditas buat sinkronisasi stok
                    
                    return { status: 'success', message: `Batch naik ke tahap ${nextTahapan}!` };
                } catch (err) {
                    return h.response({ status: 'error' }).code(500);
                }
            }
        ...
        {
            // 18. DELETE Batch Pembibitan
            method: 'DELETE',
            path: '/api/pembibitan/delete/{id}',
            handler: async (request, h) => {
                const { id } = request.params;
                try {
                    await pool.query('DELETE FROM pembibitan WHERE id = $1', [id]);
                    return { status: 'success', message: 'Batch dihapus' };
                } catch (err) { return h.response({ status: 'error' }).code(500); }
            }
        }
    ]);
